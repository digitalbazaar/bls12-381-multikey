...
export function i2osp(value, length) {
  value = BigInt(value);
  if(!(length <= 8 && Number.isSafeInteger(length))) {
    throw new Error(`"length" (${length}) must be a safe integer <= 8.`);
  }
...
export function messages_to_scalars({messages, api_id = '', ciphersuite} = {}) {
  /* Definitions:
  ...
  */
  const map_dst = TEXT_ENCODER.encode(api_id + 'MAP_MSG_TO_SCALAR_AS_HASH_');
