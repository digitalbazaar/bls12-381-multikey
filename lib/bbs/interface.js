...
export async function ProofGen({
  ...
  assertType('string', ciphersuite_id, 'ciphersuite_id');

  const ciphersuite = getCiphersuite(ciphersuite_id);
  const api_id = _createApiId(ciphersuite_id);
  ...
  */
  ...
  const proof = await CoreProofGen({
    ...
    message_scalars, disclosed_indexes, api_id, ciphersuite
  });
  return proof;
}

export async function ProofVerify({
  PK, proof, header, ph, disclosed_messages, disclosed_indexes, ciphersuite_id
} = {}) {
  assertInstance(Uint8Array, PK, 'PK');
  ...
  */
  ...
  const result = CoreProofVerify({
    PK, proof, generators, header, ph, message_scalars, disclosed_indexes,
    api_id, ciphersuite
  });
  return result;
}
...
export async function Sign({
  SK, PK, header = new Uint8Array(), messages = [], ciphersuite_id
} = {}) {
  ...
  */
  ...
  const signature = CoreSign({
    SK, PK, header, message_scalars, generators, api_id, ciphersuite
  });
  return signature;
}

export async function Verify({
  PK, signature, header, messages, ciphersuite_id
} = {}) {
  assertInstance(Uint8Array, PK, 'PK');
  assertInstance(Uint8Array, signature, 'signature');
  assertInstance(Uint8Array, header, 'header');
  ...
  const ciphersuite = getCiphersuite(ciphersuite_id);
  const api_id = _createApiId(ciphersuite);
  ...
  */
  ...
  const result = CoreVerify({
    PK, signature, generators, header, message_scalars, api_id, ciphersuite
  });
  return result;
}
