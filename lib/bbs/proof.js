 ...
 */
import {
  calculate_B, concatBytes, hash_to_scalar, i2osp, serialize, TEXT_ENCODER
} from './util.js';
...
export function ProofChallengeCalculate({
  ...
  */
  const challenge_dst = TEXT_ENCODER.encode(api_id + 'H2S_');
  ...
  */
  ...
  const c_octs = concatBytes(serialize(c_arr), i2osp(ph.length, 8), ph);
...
export function ProofFinalize({
  ...
  */
  ...
  const r3 = Fr.inv(r2);
  // `^` expressed as `Hat`
  const eHat = Fr.add(e_, Fr.multiply(e_value, challenge));
  const r1Hat = Fr.subtract(r1_, Fr.multiply(r1, challenge));
  const r3Hat = Fr.subtract(r3_, Fr.multiply(r3, challenge));
  const mHat = undisclosed_messages.map(
    (j, undisclosed) => Fr.add(m_j[j], Fr.multiply(undisclosed, challenge)));
